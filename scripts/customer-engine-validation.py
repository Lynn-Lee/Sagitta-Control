...
def run_check(
    ...
    if status in expected:
        ...
        results.append(CheckResult(label, name, False, f"HTTP {status}: {payload}"))
    return payload


def add_skip(results: list[CheckResult], target: Target, name: str, reason: str) -> None:
    results.append(
        CheckResult(f"{target.db_type}#{target.instance_id}", name, True, reason, skipped=True)
    )
...
def validate_target(
    ...
    run_check(results, client, target, "连接测试", "POST", f"/api/v1/instances/{target.instance_id}/test/")
    run_check(
        results,
        client,
        target,
    ...
    if args.sync_databases:
        ...
        add_skip(results, target, "从引擎同步库列表", "未开启 --sync-databases")
    ...
    if target.table_name:
        ...
        add_skip(results, target, "表级元数据", "未提供 table_name")
    ...
    if args.execute_readonly_query or target.run_query:
        ...
        add_skip(results, target, "只读在线查询执行", "未开启 --execute-readonly-query")
    ...
    if args.submit_workflow_boundary or target.run_mutating_workflow_check:
        ...
        add_skip(results, target, "SQL 工单审核边界", "未开启 --submit-workflow-boundary")
...
def write_reports(results: list[CheckResult], targets: list[Target], out_dir: Path) -> None:
    ...
    for result in results:
        ...
        lines.append(f"| {result.target} | {result.name} | {marker} | {detail} |")
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    print(f"客户验证后交付引擎实测完成：{passed} 通过，{skipped} 跳过，{failed} 失败。")
