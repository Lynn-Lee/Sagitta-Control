...
class DashboardService:
    @staticmethod
    async def _resolve_instance_scope(db: AsyncSession, user: dict[str, Any]) -> dict[str, Any]:
    ...
    @staticmethod
    def _apply_instance_scope(stmt: Any, scope: dict[str, Any]) -> Any:
    ...
    @staticmethod
    def _apply_instance_database_scope(stmt: Any, scope: dict[str, Any]) -> Any:
        if scope["mode"] == "instance_scope":
            instance_ids = scope.get("instance_ids") or []
            if not instance_ids:
    ...
    @staticmethod
    async def _resolve_query_scope(db: AsyncSession, user: dict[str, Any]) -> dict[str, Any]:
        return await GovernanceScopeService.resolve(db, user, "query")

    @staticmethod
    def _apply_query_scope(stmt: Any, scope: dict[str, Any]) -> Any:
    ...
    @staticmethod
    def _apply_query_apply_scope(stmt: Any, scope: dict[str, Any]) -> Any:
        return GovernanceScopeService.apply_scope(
            stmt,
            scope,
    ...
    @staticmethod
    def _apply_query_privilege_scope(stmt: Any, scope: dict[str, Any]) -> Any:
        return GovernanceScopeService.apply_scope(
            stmt,
            scope,
    ...
    @staticmethod
    async def get_query_overview(db: AsyncSession, user: dict[str, Any], days: int = 7) -> dict[str, Any]:
    ...
    @staticmethod
    async def get_instance_overview(db: AsyncSession, user: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "instance_type_distribution": [
                {"db_type": row.db_type, "count": _row_int(row, "count")} for row in type_rows
            ],
    ...
    @staticmethod
    async def _resolve_workflow_scope(db: AsyncSession, user: dict[str, Any]) -> dict[str, Any]:
        return await GovernanceScopeService.resolve(db, user, "workflow")

    @staticmethod
    def _apply_workflow_scope(stmt: Any, scope: dict[str, Any]) -> Any:
        return GovernanceScopeService.apply_scope(
            stmt,
            scope,
    ...
    @staticmethod
    def _apply_workflow_log_scope(stmt: Any, scope: dict[str, Any]) -> Any:
        return GovernanceScopeService.apply_scope(
            stmt,
            scope,
            user_col=SqlWorkflow.engineer_id,
    ...
    @staticmethod
    async def _visible_archive_workflow_ids_for_user(db: AsyncSession, user: dict[str, Any]) -> set[int]:
    ...
    @staticmethod
    async def _archive_conditions_for_user(db: AsyncSession, user: dict[str, Any]) -> list[Any]:
        if (
    ...
    @staticmethod
    async def get_archive_overview(db: AsyncSession, user: dict[str, Any], days: int = 7) -> dict[str, Any]:
        now = datetime.now(UTC)
        period_start = (now - timedelta(days=days - 1)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        ...
        def archive_stmt(*extra_conditions: Any) -> Any:
    ...
    @staticmethod
    async def get_workflow_overview(db: AsyncSession, user: dict[str, Any], days: int = 7) -> dict[str, Any]:
        now = datetime.now(UTC)
        period_start = (now - timedelta(days=days - 1)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        ...
        def workflow_stmt(*conditions: Any) -> Any:
        ...
        def workflow_log_stmt(*conditions: Any) -> Any:
            stmt = (
        ...
        return {
            ...
            "top_instances": [
                {"instance_name": row.instance_name or "未知实例", "count": _row_int(row, "count")}
                for row in top_instance_rows
            ],
            "top_databases": [
                {"db_name": row.db_name or "未知数据库", "count": _row_int(row, "count")}
    ...
    @staticmethod
    async def get_stats(db: AsyncSession, days: int = 30) -> dict[str, Any]:
        """
        Dashboard 统计。
    ...
    @staticmethod
    async def get_workflow_trend(db: AsyncSession, days: int = 7) -> list[dict[str, Any]]:
        """
        ...
        for i in range(days - 1, -1, -1):
            ...
            async def day_count_status(status_val: int, ds: datetime = day_start, de: datetime = day_end) -> int:
                stmt = (
                    select(func.count())
                    .select_from(SqlWorkflow)
                    .where(
            ...
            async def day_count_all(ds: datetime = day_start, de: datetime = day_end) -> int:
                stmt = (
                    select(func.count())
                    .select_from(SqlWorkflow)
    ...
    @staticmethod
    async def get_instance_dist(db: AsyncSession) -> list[dict[str, Any]]:
        result = await db.execute(
        ...
        return [{"db_type": row.db_type, "count": _row_value(row, "count")} for row in result]
