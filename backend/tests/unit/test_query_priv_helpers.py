...
def test_safe_load_nodes_valid_and_invalid():
    assert QP._safe_load_nodes('[{"a": 1}]') == [{"a": 1}]
    assert QP._safe_load_nodes("") == []
    assert QP._safe_load_nodes("not-json{") == []
...
def test_decorate_snapshot_fills_manager_node_only():
    ...
    out = QP._decorate_snapshot_for_applicant(nodes, applicant)
...
def test_decorate_snapshot_falls_back_to_username():
    ...
    out = QP._decorate_snapshot_for_applicant(nodes, {"id": 1, "username": "bob"})
...
def test_get_current_pending_node_picks_first_pending():
    ...
    node = QP._get_current_pending_node(apply)
...
def test_get_current_pending_node_none_when_all_done():
    ...
    assert QP._get_current_pending_node(apply) is None
...
def test_instance_access_superuser_and_global_perm():
    ...
    assert QP.user_has_instance_access({"is_superuser": True}, inst) is True
    assert QP.user_has_instance_access(
...
def test_instance_access_by_resource_group_intersection():
    ...
    assert QP.user_has_instance_access({"resource_groups": [2]}, inst) is True
    assert QP.user_has_instance_access({"resource_groups": [5]}, inst) is False
...
def test_query_bypass_admin_and_global():
    ...
    assert QP.user_has_query_bypass({"is_superuser": True}, inst) is True
    assert QP.query_bypass_reason({"is_superuser": True}, inst) == "admin"


def test_query_bypass_requires_resource_group_permission():
    inst = _instance(rg_ids=[1])
    # 有 rg 交集但缺权限 -> 不可绕过
    assert QP.user_has_query_bypass({"resource_groups": [1]}, inst) is False
    user = {"permissions": ["query_resource_group_instance"], "resource_groups": [1]}
    assert QP.user_has_query_bypass(user, inst) is True
    assert QP.query_bypass_reason(user, inst) == "resource_group_query"


def test_query_bypass_reason_empty_when_no_path():
    inst = _instance(rg_ids=[1])
    assert QP.query_bypass_reason({"resource_groups": [9]}, inst) == ""


# ── _normalize_scope_type ───────────────────────────────────

def test_normalize_scope_type_variants():
    assert QP._normalize_scope_type("instance") == ("instance", 0)
    assert QP._normalize_scope_type("table") == ("table", 2)
    assert QP._normalize_scope_type(None, table_name="t") == ("table", 2)
    assert QP._normalize_scope_type("database") == ("database", 1)
    assert QP._normalize_scope_type(None, db_name="d") == ("database", 1)
    assert QP._normalize_scope_type(None) == ("database", 1)


# ── _pg_table_candidates ────────────────────────────────────

def test_pg_table_candidates():
    assert QP._pg_table_candidates("public", "t") == ["t", "public.t"]
    assert QP._pg_table_candidates("", "t") == ["t"]
    assert QP._pg_table_candidates("public", "") == []


# ── _normalize_table_lookup_names ───────────────────────────

def test_normalize_table_lookup_empty_returns_empty():
    assert QP._normalize_table_lookup_names(_instance(), "d", "  ") == []


def test_normalize_table_lookup_pgsql_qualified():
    inst = _instance(db_type="pgsql")
    assert QP._normalize_table_lookup_names(inst, "d", "public.t") == ["t", "public.t"]


def test_normalize_table_lookup_non_pgsql_with_db():
    inst = _instance(db_type="mysql")
    assert QP._normalize_table_lookup_names(inst, "shop", "orders") == [
        "orders", "shop.orders",
    ]
...
def test_normalize_table_lookup_plain():
    inst = _instance(db_type="mysql")
    assert QP._normalize_table_lookup_names(inst, "", "orders") == ["orders"]
...
async def test_has_instance_priv_true_false():
    ...
    assert await QP._has_instance_priv(db, 1, 2) is True

    db.execute.return_value = _first_result(None)
    assert await QP._has_instance_priv(db, 1, 2) is False
...
async def test_has_db_priv_short_circuits_on_instance_priv():
    ...
    assert await QP._has_db_priv(db, 1, 2, "shop") is True
...
async def test_has_table_priv_empty_names_after_instance_miss():
    ...
    assert await QP._has_table_priv(db, 1, 2, "shop", ["  ", ""]) is False
...
async def test_has_table_priv_hits_named_table():
    db = AsyncMock()
    db.execute.side_effect = [_first_result(None), _first_result(object())]
    assert await QP._has_table_priv(db, 1, 2, "shop", ["orders"]) is True
...
async def test_has_any_table_priv_in_db_and_data_dict():
    db = AsyncMock()
    db.execute.side_effect = [_first_result(None), _first_result(object())]
    assert await QP._has_any_table_priv_in_db(db, 1, 2, "shop") is True

    db2 = AsyncMock()
    db2.execute.return_value = _first_result(None)
    assert await QP._has_any_data_dict_priv(db2, 1, 2) is False
