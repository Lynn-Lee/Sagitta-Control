...
async def test_limit_bypass_returns_requested():
    out = await QueryPrivService.get_effective_query_limit(
...
async def test_search_path_non_pgsql_none():
    assert await QueryPrivService.resolve_pg_search_path(_instance("mysql"), "d", "SELECT 1", table_refs=[]) is None


@pytest.mark.asyncio
async def test_search_path_no_tables_none():
    assert await QueryPrivService.resolve_pg_search_path(_instance("pgsql"), "d", "SELECT 1", table_refs=[]) is None


@pytest.mark.asyncio
async def test_search_path_explicit_schema_appends_public():
    out = await QueryPrivService.resolve_pg_search_path(
...
async def test_search_path_resolved_single_schema(monkeypatch):
    monkeypatch.setattr(QueryPrivService, "_get_pg_table_schema_map", AsyncMock(return_value={"t": ["app"]}))
    out = await QueryPrivService.resolve_pg_search_path(
...
async def test_search_path_ambiguous_returns_none(monkeypatch):
    monkeypatch.setattr(QueryPrivService, "_get_pg_table_schema_map", AsyncMock(return_value={"t": ["a", "b"]}))
    out = await QueryPrivService.resolve_pg_search_path(
        _instance("pgsql"), "d", "SELECT * FROM t", table_refs=[{"schema": "", "name": "t"}]
    )
    assert out is None
...
async def test_list_my_privileges_returns_rows():
    ...
    out = await QueryPrivService.list_my_privileges(db, user_id=7)
    assert out == rows
...
async def test_list_my_privileges_instance_filter():
    ...
    out = await QueryPrivService.list_my_privileges(db, user_id=7, instance_id=1)
    assert len(out) == 1
...
async def test_list_manage_privileges_builds_items(monkeypatch):
    ...
    monkeypatch.setattr(QueryPrivService, "_can_revoke_privilege", AsyncMock(return_value=True))
    ...
    meta, total, items = await QueryPrivService.list_manage_privileges(db, DATA_USER)
