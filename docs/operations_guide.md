...
> 文档版本：v3.0.0
> 适用版本：Sagitta Control v3.0.0 商业部署版 + v2-lite 授权体系
...
- 公网 HTTPS 生产环境必须配置 `AUTH_COOKIE_SECURE=true`，确保浏览器登录态 HttpOnly Cookie 只通过 HTTPS 发送；仅 HTTP 源码测试环境可临时保留 `false`。
...
- 前端 Nginx 必须保留 `Content-Security-Policy` 响应头：脚本只允许同源加载，禁止第三方 frame 嵌入，保留同源 API、WebSocket、字体和 data 图片等运行所需来源；浏览器登录态使用 HttpOnly Cookie，写操作依赖后端发放的 `csrf_token` Cookie 与前端 `X-CSRF-Token` Header 做双提交校验。
...
- 登录页版权文案展示为 `Copyright © 2026 Lynn-Lee. All rights reserved.`，其中 `Lynn-Lee` 链接到 `https://github.com/Lynn-Lee`。
...
交付与支持页提供审计、查询历史、通知日志和诊断采样的保留策略入口。第一版以配置和手动清理为主，客户有强合规要求时应在交付记录中明确保留天数、清理责任人和清理前备份策略。
...
5. 第三方登录失败时检查系统配置中的 LDAP/OAuth/OIDC 参数。
...
2. 检查 `notify` 队列是否正常。
3. 检查系统配置中的邮件、钉钉、飞书、企微参数；审批类精准通知使用企业自建应用配置，不依赖群机器人 Webhook。
4. 检查用户资料中的 `email`、`dingtalk_user_id`、`feishu_open_id`、`wecom_userid`。
5. 查看通知投递日志或 `notification_delivery_log` 中的 `sent`、`failed`、`skipped` 记录；上线前可通过外部通知账号缺失检查补齐审批相关人员的通知身份。
