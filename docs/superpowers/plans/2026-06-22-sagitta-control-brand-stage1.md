...
**Goal:** Switch the visible product brand to Sagitta Control Platform / 矢准数据库安全管控平台 while keeping the existing `sagittadb` technical project code compatible.
...
- Modify: `backend/tests/unit/test_commercial_ops.py` and focused License/system config tests for backend display-name expectations while keeping `sagittadb` project-code assertions.
- Modify: `README.md`, `docs/sagittadb_prd.md`, `docs/public_commercial_delivery.md`, `docs/user_manual.md`, `docs/operations_guide.md`, `docs/commercial_promotion_copy.md`, `docs/commercial_product_manual.md`, `docs/commercial_ops_deployment_guide.md`, and `AGENT.md` for visible naming and migration boundary.
...
- [ ] **Step 2: Run focused frontend tests and verify they fail**
  ...
  Expected: FAIL because implementation still renders `SagittaDB` and `Control with Precision`.
...
- [ ] **Step 1: Update tests first**
  Assert generated commercial metadata and reports use `Sagitta Control`, while existing `project_code` remains `sagittadb`.
- [ ] **Step 2: Run focused backend tests and verify they fail**
  ...
  Expected: FAIL where implementation still emits `SagittaDB`.
...
- [ ] **Step 2: Preserve technical compatibility names**
  Keep `sagittadb`, `SagittaDB-Enterprise`, existing Git remotes, `/opt/sagittadb/source`, `deploy/helm/sagittadb`, and historical package examples as compatibility notes unless the text describes next-version naming.
...
- [ ] **Step 3: Review remaining brand hits**
  Run: `rg -n "SagittaDB|矢准数据|Control with Precision" README.md docs AGENT.md frontend/src backend/app backend/tests`
