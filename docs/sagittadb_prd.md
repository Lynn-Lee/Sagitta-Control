...
审批类主动通知采用站内通知兜底、外部应用消息增强的双通道设计。SQL 工单、数据归档、查询权限和监控权限在提交后通知当前审批节点用户；审批流转后通知下一级审批用户；最终审批通过、驳回或取消后通知申请人。站内通知在顶部通知入口展示未读数和最近消息，外部精准推送复用用户资料中的邮箱、钉钉用户 ID、飞书 Open ID 和企业微信 UserID。
