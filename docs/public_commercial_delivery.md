...
```text
ghcr.io/<org>/sagittadb-backend:2.1.0
ghcr.io/<org>/sagittadb-frontend:2.1.0
```
...
```text
Tag: sagittadb/v2.1.0
Title: SagittaDB Enterprise v2.1.0
Assets:
  SagittaDB-Enterprise-v2.1.0.zip
  SagittaDB-Enterprise-v2.1.0.zip.sha256
```
...
```bash
wget https://github.com/<org>/<public-repo>/releases/download/sagittadb/v2.1.0/SagittaDB-Enterprise-v2.1.0.zip
unzip SagittaDB-Enterprise-v2.1.0.zip
cd SagittaDB-Enterprise-v2.1.0
...
1. 确认版本号，例如 `2.1.0`。
2. 构建并推送 `ghcr.io/<org>/sagittadb-backend:2.1.0`。
3. 构建并推送 `ghcr.io/<org>/sagittadb-frontend:2.1.0`。
4. 生成并签名商业 Manifest。
5. 渲染客户部署包。
6. 生成 zip 和 sha256。
...
12. 在公开仓库创建 `sagittadb/v2.1.0` Release。
...
- 推送到 `release/**` 时，由 `.github/workflows/commercial-release.yml` 生成 RC 候选版本，例如 `2.1.0-rc.123.abcdef0`，并推送固定版本商业镜像，但不默认同步 `Public-Releases`。
...
python scripts/render-customer-package.py \
  ...
  --package-name SagittaDB-Enterprise-v2.1.0
```
...
Edition: enterprise
Trial Days: 30
Release Tag: sagittadb/v2.1.0
Package Name: SagittaDB-Enterprise-v2.1.0.zip
Backend Image: ghcr.io/<org>/sagittadb-backend:2.1.0
Frontend Image: ghcr.io/<org>/sagittadb-frontend:2.1.0
