...
```text
ghcr.io/<org>/sagitta-control-backend:2.3.0
ghcr.io/<org>/sagitta-control-frontend:2.3.0
```
...
1. 确认版本号，例如 `2.3.0`。
2. 构建并推送 `ghcr.io/<org>/sagitta-control-backend:2.3.0`。
3. 构建并推送 `ghcr.io/<org>/sagitta-control-frontend:2.3.0`。
4. 生成并签名商业 Manifest。
5. 渲染客户部署包。
6. 生成 zip 和 sha256。
...
- 推送到 `release/**` 时，由 `.github/workflows/commercial-release.yml` 生成 RC 候选版本，例如 `2.3.0-rc.123.abcdef0`，并推送固定版本商业镜像，但不默认同步公开交付仓库。
...
- GitHub Actions 默认调度到本机 Ubuntu VM 的 self-hosted runner `sagitta-control-vm`，标签为 `[self-hosted, Linux, ARM64, sagitta-control]`；workflow 使用 GitHub API tarball 检出源码且不默认使用 GitHub 托管缓存，下载时强制 HTTP/1.1 并启用多轮重试，避免私有仓库 CI 被 hosted runner、缓存计费状态或 codeload 网络状态阻断。
...
python scripts/render-customer-package.py \
  ...
  --package-name Sagitta-Control-v2.3.0
```
...
Trial Days: 30
Release Tag: v2.3.0
Package Name: Sagitta-Control-v2.3.0.zip
Backend Image: ghcr.io/<org>/sagitta-control-backend:2.3.0
Frontend Image: ghcr.io/<org>/sagitta-control-frontend:2.3.0
