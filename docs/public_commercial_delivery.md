...
- GitHub Actions 默认调度到 JD 云 ECS 的 self-hosted runner `sagitta-control-jd`，标签为 `[self-hosted, Linux, X64, sagitta-control]`；workflow 使用 GitHub API tarball 检出源码且不默认使用 GitHub 托管缓存，避免私有仓库 CI 被 hosted runner、缓存计费状态或 JD 到 GitHub git endpoint 的网络状态阻断。
