...
```text
ghcr.io/<org>/sagitta-control-backend:2.2.3
ghcr.io/<org>/sagitta-control-frontend:2.2.3
```
...
1. 确认版本号，例如 `2.2.3`。
2. 构建并推送 `ghcr.io/<org>/sagitta-control-backend:2.2.3`。
3. 构建并推送 `ghcr.io/<org>/sagitta-control-frontend:2.2.3`。
4. 生成并签名商业 Manifest。
5. 渲染客户部署包。
6. 生成 zip 和 sha256。
...
- 推送到 `release/**` 时，由 `.github/workflows/commercial-release.yml` 生成 RC 候选版本，例如 `2.2.3-rc.123.abcdef0`，并推送固定版本商业镜像，但不默认同步公开交付仓库。
...
python scripts/render-customer-package.py \
  ...
  --package-name Sagitta-Control-v2.2.3
```
...
Trial Days: 30
Release Tag: v2.2.3
Package Name: Sagitta-Control-v2.2.3.zip
Backend Image: ghcr.io/<org>/sagitta-control-backend:2.2.3
Frontend Image: ghcr.io/<org>/sagitta-control-frontend:2.2.3
