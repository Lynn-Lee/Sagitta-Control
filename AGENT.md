...
- GitHub Actions 默认使用云 ECS 上的 repository 级 self-hosted runner `sagitta-control-ecs`，调度标签为 `[self-hosted, Linux, X64, sagitta-control]`；runner 服务目录为 `/opt/actions-runner/sagitta-control`，systemd 服务名为 `actions.runner.Lynn-Lee-Sagitta-Control.sagitta-control-ecs.service`。
