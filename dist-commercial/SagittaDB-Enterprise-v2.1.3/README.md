# SagittaDB Enterprise v2.1.3

这是 SagittaDB Enterprise 客户部署包。部署包只包含生产部署配置，应用代码通过固定版本 Docker 镜像交付。

## 镜像

- 后端：`ghcr.io/lynn-lee/sagittadb-backend:2.1.3`
- 前端：`ghcr.io/lynn-lee/sagittadb-frontend:2.1.3`
...
```bash
docker load < sagittadb-backend-2.1.3.tar
docker load < sagittadb-frontend-2.1.3.tar
docker compose up -d
```
