...
首次部署会自动进入 60 天全功能试用期；试用到期后业务功能将暂停，仅保留登录和授权管理入口。
...
```bash
./go-live-check.sh   --api-base-url http://<server>:8000   --frontend-url http://<server>/   --username <admin>   --password '<password>'
```
