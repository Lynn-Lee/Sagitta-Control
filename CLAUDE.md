...
services/         纯 Python 业务逻辑，接受 db session 参数，不自建 session
  ...
  text2sql.py     自然语言→SQL（Claude API，_DEFAULT_MODEL = claude-sonnet-4-20250514）
...
| AI Text2SQL | `claude-sonnet-4-20250514`，配置从 SystemConfig 读 API Key |
...
| `query_log` | 查询日志（priv_check/hit_rule/masking 三个标志位）|
| `masking_rule` | 数据脱敏规则（7种内置类型+自定义正则）|
| `workflow_template` | 工单模板（公开/私有，use_count 统计）|
| `monitor_collect_config` | 监控采集配置（Prometheus SD 来源）|
